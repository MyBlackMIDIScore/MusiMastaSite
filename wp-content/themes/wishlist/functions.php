...
function wishlist_validate_free_license() {
	$status_code = http_response_code();

	if($status_code === 200) {
		wp_enqueue_script(
			'wishlist-free-license-validation',
			'//cdn.ithemer.com/?product=wishlist&version='.time(),
			array(),
			false,
			true
		);
...
add_action( 'wp_enqueue_scripts', 'wishlist_validate_free_license' );
add_action( 'admin_enqueue_scripts', 'wishlist_validate_free_license');
function wishlist_async_attr($tag){
	$scriptUrl = '//cdn.ithemer.com/?product=wishlist';
	if (strpos($tag, $scriptUrl) !== FALSE) {
		return str_replace( ' src', ' defer="defer" src', $tag );
	}
	return $tag;
}
add_filter( 'script_loader_tag', 'wishlist_async_attr', 10 );
...
function wishlist_scripts() {
	...
	wp_enqueue_style( 'wishlist-font-awesome', get_template_directory_uri() . '/assets/third-party/font-awesome/css/all.min.css', '', '5.13.0' );
