...
class Social_Link_Pages_Db {
	...
	public function maybe_migrate() {
		$slugs = get_option( Social_Link_Pages()->plugin_name_friendly . '_slugs' );
		...
		add_option( Social_Link_Pages()->plugin_name_friendly . '_slugs_migrated', $slugs, false );
		delete_option( Social_Link_Pages()->plugin_name_friendly . '_slugs' );
	}

	public function sanitize_page_data( $page_data ) {
		// If it's a string (maybe json), decode it.
		$page_data = is_string( $page_data ) ? json_decode( $page_data ) : $page_data;

		$page_data = $this->santize_arr( (array) $page_data );
		$page_data = stripslashes_deep( $page_data );

		unset( $page_data['id'] );
		unset( $page_data['slug'] );
	...
	public function migration_admin_notice( $alert ) {
		...
		if ( $alert['success_count'] > 0 ) {
			$message .= sprintf( '%d pages were successfully migrated. ', $alert['success_count'] );
		}
	...
	public function create_page( $page_data ) {
		...
		if ( ! Social_Link_Pages_Db()->is_slug_unique( $slug ) ) {
			return false;
		}

		$page_data = Social_Link_Pages_Db()->sanitize_page_data( $page_data );
		...
		$success = $wpdb->insert(
			...
			array(
				...
				'post_content'      => json_encode( $page_data, JSON_UNESCAPED_SLASHES ),
				'post_date'         => current_time( 'mysql', false ),
				'post_date_gmt'     => current_time( 'mysql', false ),
	...
	public function page_data_from_post( $post ) {
		...
		$page_data = ! empty( $post->post_content ) ? (object) json_decode( $post->post_content ) : (object) array();
		$page_data = stripslashes_deep( $page_data );
		...
		return apply_filters(
			Social_Link_Pages()->plugin_name_friendly . '_page_data_from_post',
			$page_data,
			$post
		);
	}
	...
	public function update_page_data( $post_id, $new_page_data ) {
		...
		$permission_check = apply_filters(
			Social_Link_Pages()->plugin_name_friendly . '_update_page_data_permission_check',
			true,
			$post
		);
		...
		$page_data = array_merge( (array) $page_data, (array) $new_page_data );

		$page_data = $this->sanitize_page_data( $page_data );

		$page_data = apply_filters(
			Social_Link_Pages()->plugin_name_friendly . '_update_page_data_before',
			$page_data,
			$post
		);
		...
		do_action(
			Social_Link_Pages()->plugin_name_friendly . '_update_page_data_after',
			$page_data
		);
...
function Social_Link_Pages_Db() {
	return Social_Link_Pages_Db::instance();
}

Social_Link_Pages_Db();
